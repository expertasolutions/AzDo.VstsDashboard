  ...
  if(children.length > 0) {
    content.push(
      <div style={{whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis"}}>
        <Link href={build._links.web.href} target="_blank"><b>{build.buildNumber}</b></Link> <Icon iconName="BranchMerge"/><Link href={branchUrl} target="_blank">{branchName}</Link> {branchCommitControl} {(<span className="font-size-s"><Icon iconName="People"/>&nbsp;{build.requestedFor!.displayName}</span>)}
        <p>
